...
export default function RecommendedNextCarousel() {
  ...
  return (
    <section className="w-full max-w-5xl text-left">
      <h2 className="mb-4 text-2xl font-semibold text-foreground">Recommended next</h2>
      <div className="rounded-3xl border border-border bg-card p-6 text-center shadow-sm">
        <p className="text-muted-foreground">You're all caught up — keep exploring</p>
      </div>
    </section>
  );
}
      ...
      {recommendedNodes.length === 0 ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-4">
          {recommendedNodes.map((node) => (
            <Card
              key={node.id}
              className="min-w-[240px] cursor-pointer rounded-3xl border border-border bg-card transition-shadow hover:shadow-lg"
