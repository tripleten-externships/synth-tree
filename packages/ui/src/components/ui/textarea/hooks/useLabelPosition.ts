...
export function useLabelPosition({
  ...
  const [dragLabelDimensions, setDragLabelDimensions] =
    useState<LabelDimensions | null>(null);
  ...
  const labelRelativePosition = useMemo(() => {
    const edgePosition = calculateLabelEdgePosition(
      labelPosition.angle,
  ...
  const handleLabelDragMove = useCallback(
    (mouseX: number, mouseY: number) => {
      ...
      const { startAngle, startMouseAngle, centerX, centerY } =
        labelDragDataRef.current;
      const currentMouseAngle = calculateAngleFromMousePosition(
        mouseX,
        mouseY,
        centerX,
        centerY
  ...
  const handleScrollWheel = useCallback(
    (e: WheelEvent) => {
      ...
      const deltaExpansion =
        e.deltaY > 0 ? scrollWheelIncrement : -scrollWheelIncrement;
  ...
  const setExpansionDistance = useCallback(
    (distance: number) => {
