...
interface UseTextareaDimensionsReturn {
  ...
  handleResizeStart: (direction: ResizeDirection, mouseX: number, mouseY: number) => void;
  handleResizeMove: (mouseX: number, mouseY: number, containerBounds: DOMRect) => void;
...
export function useTextareaDimensions({
  ...
  // Action handlers
  const updateBoundary = useCallback((side: keyof TextareaBoundaries, value: number) => {
    // Convert boundary updates to position/dimension updates
    if (side === 'left') {
      const newWidth = boundaries.right - value;
      setPosition(prev => ({ ...prev, x: value }));
      setDimensions(prev => ({ ...prev, width: Math.max(newWidth, constraints.minWidth) }));
    } else if (side === 'right') {
      ...
      setDimensions(prev => ({ ...prev, height: Math.max(newHeight, constraints.minHeight) }));
    } else if (side === 'bottom') {
  ...
  const handleResizeStart = useCallback((direction: ResizeDirection, mouseX: number, mouseY: number) => {
    console.log('Resize start:', direction, { mouseX, mouseY, position, dimensions });
    setResizeState({
      isResizing: true,
      direction,
  ...
  const handleResizeMove = useCallback((
    ...
    if (!resizeState.isResizing || !resizeState.direction || !resizeState.startMouse || !resizeState.startPosition || !resizeState.startDimensions) return;
    ...
    if (resizeState.direction.includes('east')) {
      ...
      const newWidth = Math.max(constraints.minWidth, Math.min(constraints.maxWidth, resizeState.startDimensions.width + mouseDeltaX));
      newDimensions.width = newWidth;
      // Left edge stays fixed at startPosition.x
      console.log('East resize (mirror south):', { startWidth: resizeState.startDimensions.width, mouseDeltaX, newWidth });
    }
    ...
    if (resizeState.direction.includes('west')) {
      ...
      newPosition.x = resizeState.startPosition.x + (resizeState.startDimensions.width - newWidth);
    ...
    if (resizeState.direction.includes('south')) {
      ...
      const newHeight = Math.max(constraints.minHeight, Math.min(constraints.maxHeight, resizeState.startDimensions.height + mouseDeltaY));
      newDimensions.height = newHeight;
      // Position stays the same (top edge fixed)
      console.log('South resize:', { startHeight: resizeState.startDimensions.height, mouseDeltaY, newHeight });
    }
    ...
    if (resizeState.direction.includes('north')) {
      ...
      newPosition.y = resizeState.startPosition.y + (resizeState.startDimensions.height - newHeight);
