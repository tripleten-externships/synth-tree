 ...
 *
 * Manages drag state, prevents text selection conflicts, and implements sustained-click drag initiation
 * Uses timer-based drag detection, mouse position tracking, and edge detection to distinguish clicks from drags
 * Complex drag detection prevents accidental drags during text selection while allowing intentional moves.
...
export function useTextareaDragging({
  ...
  // Controls drag event listeners and visual feedback
  const [isDraggingActive, setIsDraggingActive] = React.useState(false);

  // Track whether we're monitoring mouse for potential drag start
  // Allows canceling drag preparation if user releases quickly (click vs drag distinction)
  const [isTrackingForDrag, setIsTrackingForDrag] = React.useState(false);

  // Track offset between mouse and textarea position during drag
  // Maintains relative position so textarea follows mouse cursor accurately
  const [dragOffset, setDragOffset] = React.useState({ x: 0, y: 0 });

  // Track when drag monitoring started
...
>>>>>>> b630920 (refactor (ST-34): Adjusted comment and improve formatting)
  ...
  // Track if user held mouse down long enough to indicate drag intent
  // Distinguishes between quick clicks and sustained holds that indicate drag intent
   ...
   *
   * Initiates drag monitoring with timer-based detection and edge collision prevention
   * Sets up 100ms timer, checks for text selection and edge proximity, tracks mouse position
   * Timer prevents accidental drags from micro-movements. Edge detection prevents conflicts with resize handles.
...
>>>>>>> b630920 (refactor (ST-34): Adjusted comment and improve formatting)
  ...
  // Handle mouse events during drag or tracking phases
  // Global listeners capture mouse movement anywhere on document
  // Allows smooth dragging even when mouse moves outside textarea bounds
  React.useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Update textarea position during active drag
      // Calculate new position based on mouse position minus initial offset
      // Maintains relative positioning so textarea follows mouse cursor accurately
      ...
      // Cancel drag if mouse moves significantly during tracking phase
      // Calculate Euclidean distance from start position
      // Distinguishes between text selection (small movements) and drag intent (larger movements)
      if (isTrackingForDrag && dragStartPosition) {
        const deltaX = Math.abs(e.clientX - dragStartPosition.x);
        const deltaY = Math.abs(e.clientY - dragStartPosition.y);
        const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);

        // Cancel drag preparation if mouse moved more than 0px
        // Any movement during tracking phase indicates text selection, not drag intent
        if (distance > 0) {
          setIsTrackingForDrag(false);
          // Clear drag timers from all textareas
          // Prevents drag initiation if user was selecting text across multiple elements
    ...
    // Resets all drag-related state and re-enables text selection
    const handleMouseUp = () => {
      ...
      // Restores normal browser behavior after drag operation
      document.body.style.userSelect = "";
      document.body.style.webkitUserSelect = "";

      // Clear any remaining drag timers
      // Ensures no timers continue running after mouse release
      const textareaElements = document.querySelectorAll("textarea");
      textareaElements.forEach((textarea) => {
        const timer = (textarea as any).__dragTimer;
        if (timer) {
...
>>>>>>> b630920 (refactor (ST-34): Adjusted comment and improve formatting)
  ...
  // Return drag state and handler for use in textarea component
  // Provides controlled interface for drag functionality
  return {
    isDraggingActive,
    isSustainedClick,
    isTrackingForDrag,
