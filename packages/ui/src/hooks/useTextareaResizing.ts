 ...
 * TypeScript ensures only valid resize directions are used throughout the system
 */
 ...
 *
 * Manages resize operations, cursor feedback, and maintains size/position constraints
 * Tracks mouse position relative to initial bounds, calculates new dimensions based on resize direction
 * Complex resize logic handles 8 directions with proper constraint enforcement.
...
export function useTextareaResizing({
  ...
  // Track whether resize operation is currently active
...
>>>>>>> b630920 (refactor (ST-34): Adjusted comment and improve formatting)
  ...
  // Refs for synchronous access during mouse events
  // Mouse event handlers need immediate values without waiting for React state updates
  ...
  // Ensures mouse event handlers always have latest values
  React.useEffect(() => {
   ...
   *
   * Sets up resize state and attaches global mouse listeners for smooth resizing
   * Captures initial bounds, sets resize direction, handles mouse movements with constraint validation
   * Global listeners allow resizing even when mouse moves outside original bounds.
  ...
  const handleResizeStart = React.useCallback(
    (e: React.MouseEvent, direction: ResizeDirection) => {
      ...
      // Capture initial textarea bounds for relative calculations
      // Mouse movements are calculated relative to initial position, not absolute coordinates
      ...
      // Sets up the resize operation with proper direction tracking
       ...
       *
       * Calculates new dimensions and position based on resize direction and mouse position
       * Uses directional string matching (includes "east", "west", etc.) to determine resize behavior
       * Supports 8-way resizing with proper constraint enforcement. Position adjustments prevent
      ...
      const handleMouseMove = (moveEvent: MouseEvent) => {
        ...
        // Start with current values and modify based on resize direction
        let newWidth = currentDimensions.width;
        let newHeight = currentDimensions.height;
        let newX = currentPosition.x;
        let newY = currentPosition.y;

        // Handle east-directed resizing (right edge)
        // Calculate width based on mouse position relative to parent container
        // East resize changes width while keeping left edge fixed. Mouse-relative-to-parent
        ...
        // Handle west-directed resizing (left edge)
        // Adjusts both width and X position to resize from left edge
        // West resize moves the textarea left while shrinking width. Position adjustment
        ...
        // Handle south-directed resizing (bottom edge)
        // Simple height calculation based on mouse Y position
        // South resize only affects height, keeping top edge fixed
        ...
        // Handle north-directed resizing (top edge)
        // Adjusts both height and Y position to resize from top edge
        // North resize moves textarea up while shrinking height. Position adjustment
        ...
        // Updates both size and position simultaneously for smooth resize experience
        setTextareaDimensions({ width: newWidth, height: newHeight });
        setTextareaPosition({ x: newX, y: newY });
      };

      // Clean up resize operation on mouse release
      // Resets resize state and removes event listeners to prevent memory leaks
      const handleMouseUp = () => {
...
>>>>>>> b630920 (refactor (ST-34): Adjusted comment and improve formatting)
  ...
  // Return resize state and handlers for use in textarea component
  // Provides controlled interface for resize functionality
  return {
    isResizingActive,
    currentResizeDirection,
    hoveredResizeEdge,
