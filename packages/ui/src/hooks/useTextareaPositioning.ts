 ...
 *
 * WHAT: Provides state management for textarea x,y coordinates and automatic centering
 * HOW: Uses useState for position state, useRef for synchronous access, and useEffect for initial centering
 * WHY: Separate positioning logic from component to enable reusability and testing.
...
export function useTextareaPositioning({
  ...
  // WHAT: Keep ref in sync with state changes
  // WHY: Ensures ref always reflects the latest position state for mouse event handlers
  React.useEffect(() => {
    textareaPositionRef.current = textareaPosition;
  }, [textareaPosition]);

  // WHAT: Center textarea in viewport on initial mount
  // HOW: Calculates center position based on viewport size and initial dimensions
  ...
  // WHAT: Return position state, setter, and synchronous ref
  // WHY: Provides both async state updates (for React rendering) and sync ref access (for mouse events)
  return {
    textareaPosition,
    setTextareaPosition,
    textareaPositionRef,
